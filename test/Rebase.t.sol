...
contract RebaseTokenTest is Test {
    ...
    function addRewardsToVault(uint256 rewartAmount) public {
        // send some rewards to the vault using the receive function
       (bool sucess,) = payable(address(vault)).call{value: rewartAmount}("");
    }
    ...
    function testRedeemAfterTimeHasPassed(uint256 depositAmount, uint256 time) public {
       time = bound(time, 1000, type(uint32).max); // this is a crazy number of years - 2^96 seconds is a lot 
